package com.ald.bigdata.common.trend.service;
...
public class TrendService {
    ...
    public Pair<List, List> tableData(TrendQueryVo trendQueryVo) {
        ...
        /*
         * 优化：1.if else逻辑中存在四处相同的代码，去除冗余，提高可读性。
         *      2.使用StringUtils.equals优化空指针。
         */
        if (StringUtils.equals(trendQueryVo.getDataType(), "1")) {
            pair = getHourData(trendQueryVo);
        } else if (StringUtils.equals(trendQueryVo.getDataType(), "2")) {
            pair = getDayData(trendQueryVo);
        } else if (StringUtils.equals(trendQueryVo.getDataType(), "3")) {
            pair = getWeekData(trendQueryVo);
        } else if (StringUtils.equals(trendQueryVo.getDataType(), "4")) {
            pair = getMonthData(trendQueryVo);
        }
        List leftData = null;
        List rightData = null;
